...
data class QaFindingsDocument(val schemaVersion: Int, val volume: Int, val chapter: Int, val findings: List<QaFinding>) {
    ...
    fun activeBlocking(editorContentSha256: String) = active(editorContentSha256).filter { it.severity == "blocking" }
    ...
    fun parse(raw: String): QaFindingsDocument {
        ...
        val findings = buildList {
            for (i in 0 until array.length()) {
                ...
                add(QaFinding(item.getString("id"), item.optString("locator").takeUnless { it.isBlank() }.orEmpty(), item.getString("category"), item.getString("severity"), item.getString("message"), item.optBoolean("overridable", false), override))
            }
        }
        return QaFindingsDocument(root.getInt("schema_version"), root.getInt("volume"), root.getInt("chapter"), findings)
    }
    fun serialize(document: QaFindingsDocument): String {
        val root = JSONObject().put("schema_version", document.schemaVersion).put("volume", document.volume).put("chapter", document.chapter)
